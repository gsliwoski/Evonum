...
#		print ("="*30)
    ...
    # Prune the population of solvers of those that had the lowest fitness
    # scores
...
#			print ("Population: %s\tMax Population: %s\tNeed to kill: %s" % (len(self._solvers), self._max_solvers, len(self._solvers)-self._max_solvers))
        ...
        for item in self._forces:
            ...
            # adapt first.
            if item.getType() == "Dynamic" and self._current_day > 100:
...
#				print (item[0].getName()+"\t"+str(item[1]))
    ...
    # End the world in days days, ramp down the maximum population each day to
    # reach final collection of destination of the fittest solvers at the end.
