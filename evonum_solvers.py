...
class SolverFactory(object):
    ...
    @classmethod
    #TODO: Not finished. Will eventually allow defining solver properties at
    #      run-time but for now pre-defined solver must be imported by dict
    def DefineSolver(self, options):
        if options["Type"] == "Small":
            new_solver = SmallSolver()
            try:
                new_solver._name = options["Name"]
            except:
                pass
            try:
                new_solver._unique = True if options[
                    "Unique"] == "True" else False
            except:
                pass
            try:
                new_solver._lifespan = int(options["Lifespan"])
            except:
                pass
            ...
            try:
                new_solver._spread = options["Spread"]
            except:
                pass
            try:
                new_solver._total_modules = len(options["Modules"])
                for item in options["Modules"]:
                    new_solver.addModule(item.split(
                        ",")[0], item.split(",")[1])
            except:
                pass
            try:
                new_solver._module_mutation_chance = float(
                    options["ModuleMutationChance"])
            except:
                pass
            try:
                new_solver._property_mutation_chance = float(
                    options["PropertyMutationChance"])
            except:
                pass
    ...
    @classmethod
    # Import solver based on its exported dict
...
class SolverInterface(object):
    ...
    # Resets all properties to default, returns nothing
    def hardReset(self):  
        error()
    ...
    # Returns floating-point fitness score
    def getFitness(self):  
        error()
...
class SmallSolver(SolverInterface):
    ...
    def __init__(self, name="Unnamed Linear Solver"):
        ...
        # Index correlations: 0 = spread, 1 = total_modules, 2 =
        # module_mutation_chance, 3 = property_mutation_chance; These are
        ...
        # Chance for a module that will be mutated to swap out with a new
        # module of different subtype instead of mutating.
        self._swap_module_mutation_chance = 15
        # Chance that two modules of the same subtype will merge
...
#					print ("swapping out "+self._modules[x].getSubtype())
                    if not self._unique:
                        self._modules[x] = ModuleFactory.generateUniqueModule(
                            "Fitness", [self._modules[x].getSubtype()])
                    else:
                        present_subtypes = [y.getSubtype()
...
#			print ("%s: Mutating Spread" % self._name)
            ...
            # mutation
            self._spread = Mutations.GaussianMutation(
                self._spread, 1 if self._spread == 0 else self._spread)
            if self._spread < 0:
                self_spread = 0
    ...
    def addModule(self):
        if not self._unique:
            self._modules.append(
                ModuleFactory.generateNewModule("Fitness", "Random"))

        else:
            present_subtypes = [x.getSubtype() for x in self._modules]
            self._modules.append(ModuleFactory.generateUniqueModule(
                "Fitness", present_subtypes))
...
#				print (self.getDescription())
                new_modules = ModuleFactory.mergeFitnessModules(
                    self._modules[i], self._modules[module_subtypes[item.getSubtype()]])
                self._modules[module_subtypes[item.getSubtype()]] = new_modules[
                    0]
