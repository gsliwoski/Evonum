        ...
        for line in script:
            ...
            if line.startswith("World"):
                ...
                try:
                    ...
                    raise IndexError("Error: World: must be followed with #max solver, #max forces, #chance to survive prune")
                except ValueError:
                    raise TypeError("Error: Bad type given for world setting(s).")
                self._worlds[-1].importAttributes(settings) #TODO: Complete ability to have multiple worlds running with different solver conditions.
#                try:
...
#                self._worlds[-1].updateSolverConditions(solver_conditions)
                ...
                try:
                    ...
                    print("Error: Force: must be followed with number, type, subtype, conditions. Force skipped.")
                    continue
                try:
                    total = int(sets[0])
                    force_type = sets[1]
                    ...
                    print(force_type,force_subtype,force_conditions)
                except IndexError:
                    print("Error: Force: must be followed with number, type, subtype, conditions. Force skipped.")                        
                    continue
                except ValueError:
                    print("Error: Bad number of forces, force skipped.")
                    continue
                for x in range(0, int(sets[0])):
                    self._worlds[-1].addForce(force_type, force_subtype, force_conditions)
                    print ("%d fitness forces added to world of type %s and subtype %s" % (total,force_type, force_subtype))
                ...
                try:
                    ...
                    print("Error: Refresh Solvers: must be followed with #solvers to refresh and #days between refresh.")
                ...
                try:
                    ...
                    raise ValueError("Error: Run: must be followed by int number of days to run.")
                ...
                try:                                
                    ...
                    print("Error: End: must be followed by target population and duration of end. Skipping end.")
                else:
...
#                print ("blocks: %d, block_size: %d, remainder: %d" % (blocks, block_size, remainder))
        ...
        for pos, world in enumerate(self._worlds):
            ...
            if self._export is not None:
                if len(solvers)>0:
                    outfile = open(self._export+"world"+str(pos+1)+"_solvers.json", "w")
                    outfile.write("[\n")
