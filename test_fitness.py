...
class TestFitnessForceCreation:
    ...
    def test_create_dynamic_equation_fitess(self):
        ...
        dynamic_equation_fitness = createFitnessForce(
            "Dynamic", "Equation", conditions)
    ...
    def test_create_force_badtype_conditions(self):
        with pytest.raises(TypeError):
            position_badtype_conditions = createFitnessForce(
                "Simple", "Position", 1)
        with pytest.raises(TypeError):
            simple_equation_badtype_conditions = createFitnessForce(
                "Simple", "Equation", False)
        with pytest.raises(TypeError):
...
class TestBeginDay:
    ...
    def test_position_fitness_force(self):
        position_force = createFitnessForce(
    ...
    def test_simple_equation_fitness_force(self):
        equation_force = createFitnessForce(
            "Simple", "Equation", "x + 5*pow(x,2), 1, 10")
    ...
    def test_dynamic_equation_fitness_force(self):
        dynamic_equation_force = createFitnessForce(
            "Dynamic", "Equation", "x + 5*pow(x,2), 1, 10")
        dynamic_equation_force.beginDay()
        variable = dynamic_equation_force._current_condition
        expected = variable + 5 * pow(variable, 2)
        assert round(
            dynamic_equation_force._current_expected) == round(expected)

    def test_bad_equation_simple(self):
        bad_equation_force = createFitnessForce(
            "Simple", "Equation", "log(x),-1,-5")
        with pytest.raises(ValueError):
            bad_equation_force.beginDay()

    def test_unrecognizable_equation_simple(self):
        unrecognizable_equation_force = createFitnessForce(
            "Simple", "Equation", "siiiwl(x),1,2")
        with pytest.raises(NameError):
            unrecognizable_equation_force.beginDay()

    def test_bad_equation_dynamic(self):
        bad_equation_dynamic = createFitnessForce(
            "Simple", "Equation", "log(x,5),-1,0")
        with pytest.raises(ValueError):
            bad_equation_dynamic.beginDay()

    def test_unrecognizable_equation_dynamic(self):
        unrecognizable_equation_dynamic = createFitnessForce(
            "Dynamic", "Equation", "roietioeo,2,3")
        with pytest.raises(NameError):
